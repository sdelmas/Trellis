...
Curate `implement.jsonl` and `check.jsonl` so the Phase 2 sub-agents get the right spec/research context. These files are created empty on `task create`; your job here is to fill in real entries.
